...
```{r import-data}
path_to_proteinGroups <- system.file("extdata/example_1_proteinGroups.txt", package = "sev")

se <- sev::se_read_in(file = path_to_proteinGroups, sep = "_rep_")
...
```{r import-data2}
path_to_design <- system.file("extdata/example_1_design.txt", package = "sev")

design <- read.csv(file = path_to_design)

design

se <- sev::se_read_in(file = path_to_proteinGroups, experimental_design = design)
...
```{r save-results}
save(isee_object, file = "isee_object.rda")

load("isee_object.rda")
```
